...
BinaryPacker["readHead"] = BinaryPacker_readHead; // BinaryPacker.readHeader(source:Uint8Array, cursor:Integer = 0):Object
BinaryPacker["writeHead"] = BinaryPacker_writeHead; // BinaryPacker.writeHeader(source:Uint8Array, cursor:Integer, bodyLength:Integer, formatID:Integer):cursor
...
//}@dev
    ...
    var head = BinaryPacker_readHead(source);

    if (head) {
        var formatID = head["formatID"];
        var packer = global["BinaryPacker" + _toHex(formatID)];

        if (packer) {
            return packer["unpack"](source, head);
        }
        throw new TypeError("unknown format: " + _toHex(formatID));
    }
...
//{@dev
    ...
    $valid($type(formatID,   "Integer"),    BinaryPacker_writeHead, "formatID");
//}@dev
...
function BinaryPacker_getBodyLength(source,     // @arg Any - source data
                                    formatID) { // @arg Integer - 0x0001 - 0xffff
                                                // @ret Integer
//{@dev
    $valid($type(formatID, "Integer"), BinaryPacker_getBodyLength, "formatID");
//}@dev

    var packer = global["BinaryPacker" + _toHex(formatID)];

    if (packer) {
        return packer["getBodyLength"](source);
    }
    throw new TypeError("unknown format: " + _toHex(formatID));
}

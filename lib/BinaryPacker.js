...
BinaryPacker["getBytes"] = BinaryPacker_getBytes; // BinaryPacker.getBytes(source:Any, formatID:Integer):Integer
...
function BinaryPacker_getBytes(source,     // @arg Any - source data
                               formatID) { // @arg Integer - 0x0001 - 0xffff
                                           // @ret Uint8Array|null
//{@dev
    $valid($type(formatID, "Integer"), BinaryPacker_getBytes, "formatID");
//}@dev

    var packer = global["BinaryPacker" + _toHex(formatID)];

    if (packer) {
        return packer["getBytes"](source);
    }
    throw new TypeError("unknown format: " + _toHex(formatID));
}
